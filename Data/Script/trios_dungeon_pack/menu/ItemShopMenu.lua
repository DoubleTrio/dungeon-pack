...
function ItemShopMenu:initialize(title, items, filter, confirm_action, refuse_action, confirm_button, menu_width,
                                        include_equips, max_choices, label)
...
function ItemShopMenu:load_slots()
  local list = {}
...
function ItemShopMenu:SortCommand()
...
function ItemChosenMenu:initialize(slots, parent, confirm_text, confirm_action, label)
  ...
  local options = {
    ...
    { STRINGS:FormatKey("MENU_INFO"),   true, function() _MENU:AddMenu(RogueEssence.Menu.TeachInfoMenu(self.first_item),
        false) end },
...
function ItemShopMenu.run(title, filter, confirm_text, includeEquips, max_choices)
  local ret = {}
  local choose = function(list) ret = list end
  local refuse = function() _MENU:RemoveMenu() end
  local menu = ItemShopMenu:new(title, filter, choose, refuse, confirm_text, 176, includeEquips, max_choices)
