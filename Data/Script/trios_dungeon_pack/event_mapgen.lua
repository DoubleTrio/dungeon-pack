...
function ZONE_GEN_SCRIPT.EmberfrostSwitchUp(zoneContext, context, queue, seed, args)
    ...
    elseif (zoneContext.CurrentID % interval) == 0 and zoneContext.CurrentID ~= 0 then
        ...
        active_effect.OnMapStarts:Add(-5,
            RogueEssence.Dungeon.SingleCharScriptEvent("AddSwitchSegmentStairs", Serpent.line({
    ...
    if (is_shimmering or zoneContext.CurrentID == 2 or zoneContext.CurrentID >= 20) and not is_beginning then
        local SHIMMERING_EVENTS = {{
            ...
            effect = function()
                ...
                active_effect.BeforeStatusAdds:Add(-1, PMDC.Dungeon
                    .StateStatusCheck(bad_status_type, string_key, status_event))
            end,
            ...
            effect = function()
                ...
                local status_event = CreateShimmeringStatusEvent()
                active_effect.BeforeStatusAdds:Add(0, PMDC.Dungeon
                    .StatChangeCheck(stats, string_key, true, false, false, status_event))
            end,
        ...
        local shimmering_status = LUA_ENGINE:MakeGenericType(DefaultMapStatusStepType, {MapGenContextType},
            {"default_weather", shimmer_selection})
        local priority = RogueElements.Priority(-6)
        queue:Enqueue(priority, shimmering_status)
        local ev = SHIMMERING_EVENTS[index + 1]
        ev.effect()
        active_effect.OnMapStarts:Add(1, RogueEssence.Dungeon.SingleCharScriptEvent("LogShimmeringEvent", Serpent.line({
            StringKey = ev.string_key
        })))
    end
...
function ZONE_GEN_SCRIPT.SpawnStoryNpc(zoneContext, context, queue, seed, args)
    ...
    if GAME:InRogueMode() then
        ...
        SPAWNS = {{
            ...
            Statuses = {"crystal_defense", "crystal_attack"},
            Level = 25,
            Emote = 0
        }, {
            Species = "ribombee",
    ...
    for _, entry in ipairs(SPAWNS) do
        if curr_floor + 1 == entry.Floor then
            ...
            post_mob.SpawnFeatures:Add(PMDC.LevelGen.MobSpawnLuaTable(Serpent.line({
                NPC = true
            })))
            ...
            mobPlacement.Filters:Add(PMDC.LevelGen.RoomFilterConnectivity(
                PMDC.LevelGen.ConnectivityRoom.Connectivity.Main))
