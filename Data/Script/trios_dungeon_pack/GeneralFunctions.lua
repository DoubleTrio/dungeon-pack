...
function GeneralFunctions.LookAround(chara, rotations, turnframes, emote, allDirections, sound, startLeft, enddir)
  ...
  if enddir ~= Direction.None and enddir ~= dir then --if a direction to end on was specified and we aren't facing that way, turn there
    ...
    GAME:WaitFrames(turnframes * 2)   --wait for some time based off how long it could have taken to turn if we dont turn at the end
  end
end
...
function GeneralFunctions.MoveCamera(x, y, speed)
  ...
  local cameraFrames = GeneralFunctions.CalculateCameraFrames(GAME:GetCameraCenter().X, GAME:GetCameraCenter().Y, x, y,
    speed)
...
function GeneralFunctions.HeroDialogue(chara, str, emotion)
  ...
  UI:SetSpeaker('', false, chara.CurrentForm.Species, chara.CurrentForm.Form, chara.CurrentForm.Skin,
    chara.CurrentForm.Gender)
  ...
  if hero == nil or partner == nil then
    ...
    for i = assembly_count, 1, -1 do
      ...
      elseif tbl.Importance == 'Partner' then
        ...
        partner = GAME:GetPlayerPartyMember(GAME:GetPlayerPartyCount() - 1)
      end
    end
  end
...
function GeneralFunctions.Hop(chara, anim, height, duration, pause, sound)
  ...
  height + 1                       --one more duration than height makes the hop hang for a frame at the peak, like explorers.
...
function GeneralFunctions.DoubleHop(chara, anim, height, duration, pause, sound)
  ...
  GROUND:CharSetAction(chara,
    RogueEssence.Ground.HopGroundAction(chara.Position, chara.Direction, animId, height, duration))
...
function GeneralFunctions.PoseAndWait(chara, anim)
  local pose_action = RogueEssence.Ground.PoseGroundAction(chara.Position, chara.Direction,
    RogueEssence.Content.GraphicsManager.GetAnimIndex(anim))
...
function GeneralFunctions.ReversePoseAndWait(chara, anim)
  local pose_action = RogueEssence.Ground.ReverseGroundAction(chara.Position, chara.LocHeight, chara.Direction,
    RogueEssence.Content.GraphicsManager.GetAnimIndex(anim))
  GROUND:CharSetAction(chara, pose_action)
  GROUND:CharWaitAction(chara, pose_action)
  GROUND:CharEndAnim(chara)
...
function GeneralFunctions.RewardItem(itemID, money, amount)
  ...
  if money then
    UI:WaitShowDialogue("Team " ..
    GAME:GetTeamName() ..
    " received " .. "[color=#00FFFF]" .. itemID .. "[color]" .. STRINGS:Format("\\") .. ".[pause=40]")
    ...
    UI:WaitShowDialogue(STRINGS:Format("Team " ..
    GAME:GetTeamName() .. " received [a/an] " .. item:GetDisplayName() .. ".[pause=40]"))
...
function GeneralFunctions.RankUp(leftover_points)
  ...
  UI:WaitShowDialogue("Team " ..
  GAME:GetTeamName() ..
  " went up in rank from the\n[color=#FFA5FF]" ..
  current_rank:gsub("^%l", string.upper) ..
  " Rank[color] to the [color=#FFA5FF]" .. next_rank:gsub("^%l", string.upper) .. " Rank[color]!")

  --notify of bag size increase
  if _DATA:GetRank(current_rank).BagSize < _DATA:GetRank(next_rank).BagSize then
    UI:WaitShowDialogue("The number of items you can store in your Treasure Bag has increased from [color=#00FFFF]" ..
    tostring(_DATA:GetRank(current_rank).BagSize) ..
    "[color] to [color=#00FFFF]" .. tostring(_DATA:GetRank(next_rank).BagSize) .. "[color].")
  end
  ...
  SOUND:PlayFanfare("Fanfare/Item")
  UI:WaitShowDialogue("For advancing in rank,[pause=10] your team was awarded a " ..
  item:GetDisplayName() .. ".[pause=40]")
...
function GeneralFunctions.StartConversation(target, dialogue, emotion, npcTurn, changeNPCanimation, changeSpeaker,
...
function GeneralFunctions.StartPartnerYesNo(dialogue, emotion, heroTurn, defaultToNo)
  ...
  local coro1 = TASK:BranchCoroutine(function()
    UI:ChoiceMenuYesNo(dialogue, defaultToNo)
    UI:WaitForChoice()
...
function GeneralFunctions.DeathFadeOutDialogue(chara, dialogue, emotion)
  ...
  UI:WaitShowDialogue(dialogue)
  UI:SetAutoFinish(true)
  UI:WaitShowTimedDialogue(string.gsub(dialogue, "%[pause=0%]", "") .. "[script=0]", 60,
    { function() return GAME:FadeInFront(false, 60) end })                                                                                   --remove pause=0 to stop unneeded pauses
  UI:SetAutoFinish(false)
  ...
  while state > -1 do
    if state == 0 then
      if has_ongoing_mission then
        ...
        UI:ChoiceMenuYesNo(
        "You have no more ongoing missions beyond this point.\nWould you like to leave the dungeon now?", false)
        UI:WaitForChoice()
        local leave_dungeon = UI:ChoiceResult()
        if leave_dungeon then
          state = -1
...
function GeneralFunctions.PagedChoiceMenu(message, choices, defaultchoice, cancelchoice)
  ...
  if choice_amount > submenu_length then
    ...
    while not continue do
      UI:BeginChoiceMenu(message, choice_submenus[current_submenu], default_cursor_option,
        # choice_submenus[current_submenu])
      UI:WaitForChoice()
      result = UI:ChoiceResult()
