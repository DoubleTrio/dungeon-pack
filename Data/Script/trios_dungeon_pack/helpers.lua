...
-- end
  ...
  SaveCharacter = function(chara, key)
    ...
    for ii = 0, RogueEssence.Dungeon.CharData.MAX_SKILL_SLOTS - 1 do
      ...
      if skill ~= nil then table.insert(c_data.Skills, skill) end
    end
    ...
    for battle_event in luanet.each(chara.ActionEvents) do
      if (LUA_ENGINE:TypeOf(battle_event) ==
            luanet.ctype(BattleScriptEventType)) then
        local event = {}
  ...
  LoadCharacter = function(key)
    ...
    charData.BaseForm = RogueEssence.Dungeon.MonsterID(c_data.BaseForm
      .Species,
      c_data.BaseForm.Form,
      c_data.BaseForm.Skin,
      NumToGender(
        c_data.BaseForm
        .Gender))
    ...
    for i, skill in ipairs(c_data.Skills) do
      if i <= RogueEssence.Dungeon.CharData.MAX_SKILL_SLOTS then
        charData.BaseSkills[i - 1] =
            RogueEssence.Dungeon.SlotSkill(skill)
      end
    end
    ...
    local metSegLoc = RogueEssence.Dungeon.SegLoc(
      c_data.MetLoc.StructID.Segment,
      c_data.MetLoc.StructID.ID)
    charData.MetLoc = RogueEssence.Dungeon.ZoneLoc(c_data.MetLoc.ID,
      metSegLoc,
      c_data.MetLoc.EntryPoint)
    charData.DefeatAt = c_data.DefeatAt

    local defeatSegLoc = RogueEssence.Dungeon.SegLoc(c_data.DefeatLoc
      .StructID.Segment,
      c_data.DefeatLoc
      .StructID.ID)
    charData.DefeatLoc = RogueEssence.Dungeon.ZoneLoc(c_data.DefeatLoc.ID,
      defeatSegLoc,
      c_data.DefeatLoc
      .EntryPoint)
    ...
    for _, event in ipairs(c_data.ActionEvents) do
      local battleEvent = RogueEssence.Dungeon.BattleScriptEvent(
        event.Script, event.ArgTable)
    ...
    local invItem = RogueEssence.Dungeon.InvItem(
      c_data.EquippedItem.ID, c_data.EquippedItem.Cursed, c_data.EquippedItem.Amount, c_data.EquippedItem.Price)
