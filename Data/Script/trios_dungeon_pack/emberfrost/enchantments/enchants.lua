...
function GetSkillFromContext(context)
  return context.User.Skills[context.UsageSlot].Element.SkillNum
end
...
PandorasItems = EnchantmentRegistry:Register({
  ...
  get_description = function(self)
    return string.format(
      "Gain %s random %s, %s, and %s. At the start of each floor, any non-held items are randomized (except food, loots, shards, and treasures)",
      M_HELPERS.MakeColoredText(tostring(self.amount), PMDColor.Cyan),
...
Rationalize = EnchantmentRegistry:Register({
  ...
  get_progress_texts = function(self)
    local normal_type = _DATA:GetElement("normal")
    local icon = normal_type:GetIconName()
